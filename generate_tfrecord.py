...
Usage:
  ...
  # Create train data:
  python generate_tfrecord.py --csv_input=data/train_labels.csv  --output_path_train=train.record --output_path_test=test.record
...
flags.DEFINE_string('output_path_train', '', 'Path to output train TFRecord')
flags.DEFINE_string('output_path_test', '', 'Path to output test TFRecord')
FLAGS = flags.FLAGS
...
def main(_):
    ...
    for output in outputs:
        writer = tf.python_io.TFRecordWriter(output['filename'])
        grouped = split(output['examples'], 'filename')
        ...
        print('Successfully created the TFRecords: {} with {} data'.format(output_path, output['dataLen']))
